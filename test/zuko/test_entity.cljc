(ns zuko.test-entity
  (:require [zuko.entity.writer :refer [string->triples entities->triples entity-update->triples ident-map->triples ref->entity]]
            [zuko.entity.reader :refer [store->entities ident-map->triples ref->entity]]
...
(deftest test-encode
  (let [m1 (entities->triples (test-helper/new-store)
                          [{:prop "val"}])
        m2 (entities->triples (test-helper/new-store)
                          [{:prop "val", :p2 2}])
        m3 (entities->triples (test-helper/new-store)
                          [{:prop "val", :p2 22, :p3 [42 54]}])
        m4 (entities->triples (test-helper/new-store)
                          [{:prop "val"} {:prop "val2"}])
        m5 (entities->triples (test-helper/new-store)
                          [{:prop "val"
                            :arr [{:a 1} {:a 2} ["nested"]]}])]
    (is (= [[:test/n1 :db/ident :test/n1]
            [:test/n1 :naga/entity true]
...
(defn round-trip
  [data]
  (let [m (entities->triples empty-store data)
        new-db (store/assert-data empty-store m)]
    (set (store->entities new-db))))
...
(defn generate-diff
  [o1 o2]
  (let [triples (entities->triples empty-store [o1])
        ...
        [additions retractions] (entity-update->triples st id o2)]
...
(deftest test-ref->entity
  (let [data {:id "1234" :prop "value" :attribute 2}
        m (entities->triples empty-store [data])
        store' (store/assert-data empty-store m)
        ref (get-node-ref store' "1234")
        store (store/assert-data store' [[ref "Connected_To" ref]])
        obj1 (ref->entity store ref)
        obj2 (ref->entity store ref #{"Connected_To"})]
    (is (= data obj1))
    (is (= data obj2))))


#?(:clj
#_(deftest test-multi-update
  (let [graph
        ...
        new-store (if-let [n (get-node-ref store id)]
                    (let [[assertions retractions] (entity-update->triples store n m)
